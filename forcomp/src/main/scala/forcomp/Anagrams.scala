...
object Anagrams {
 ...
 val dictionary: List[Word] = loadDictionary
  ...
  def sentenceAnagrams(sentence: Sentence): List[Sentence] = {
    ...
    def isValidSentence(sentence: Sentence): Boolean = (sentenceOccurrences(sentence)).toSet == sentenceOccurrenceList.toSet
   ...
   def getSentenceList(occurrenceList: Occurrences): List[Sentence] = {
     if(occurrenceList.isEmpty)
       ...
       val listOfSentenceLists: List[List[Sentence]] = 
        ...
        val finalList: List[Sentence] = listOfSentenceLists.flatten
        finalList
      }      
    }
   
   val intermediateList: List[Sentence] = getSentenceList(sentenceOccurrenceList) filter (sentence => isValidSentence(sentence))
   val finalList: List[Sentence] = intermediateList
   finalList
  }
}
